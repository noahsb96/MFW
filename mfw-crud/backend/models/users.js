...
UserSchema.pre("save", async function (next) {
  // pre is a hook by mongo that will operate before the schema is saved to the database. The async function will be a promise based function. The function calls next to call the middleware inside the function.
...
UserSchema.statics.login = async function (email, password) {
  // statics is used to define a function that exist directly on the model. This is a login function that is going to be on the model to find the user by email and authorize the password.
  ...
  if (user) {
    ...
    if (auth) {
      // if the entered password is the same as the user schema password
